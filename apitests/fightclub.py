...
if(refetch == 'y'):
  target = open('threadscrape_' + board + '.txt', 'w')
  target.truncate()
  ...
  for i in range(1,length):
    ...
    for j in range(1,num_threads):
      ...
      thread_response = requests.get("https://a.4cdn.org/" + board + "/thread/" + str(thread_num) + ".json")
...
target = open('threadscrape_' + board + '.txt', 'r')
dataset = target.read()
#print(dataset)
